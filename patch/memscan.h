...
#define CHECK_QWORD_GT(dst,src,flag) asm volatile("movaps (%2), %%xmm0\n\t" \
		...
		: "r"(dst), "r"(src) \
		: "xmm0", "xmm1", "xmm3")
#define CHECK_BYTE_LE(dst,src,flag) asm volatile("movaps (%2), %%xmm1\n\t" \
		"movups (%1), %%xmm0\n\t" \
		"pcmpgtb %%xmm0, %%xmm1\n\t" \
		"movaps %%xmm1, %%xmm3\n\t" \
		"pshufd $0xee, %%xmm1, %%xmm3\n\t" \
...
#define CHECK_WORD_LE(dst,src,flag) asm volatile("movaps (%2), %%xmm1\n\t" \
		"movups (%1), %%xmm0\n\t" \
		"pcmpgtw %%xmm0, %%xmm1\n\t" \
		"movaps %%xmm1, %%xmm3\n\t" \
		"pshufd $0xee, %%xmm1, %%xmm3\n\t" \
...
#define CHECK_DWORD_LE(dst,src,flag) asm volatile("movaps (%2), %%xmm1\n\t" \
		"movups (%1), %%xmm0\n\t" \
		"pcmpgtd %%xmm0, %%xmm1\n\t" \
		"movaps %%xmm1, %%xmm3\n\t" \
		"pshufd $0xee, %%xmm1, %%xmm3\n\t" \
...
#define CHECK_QWORD_LE(dst,src,flag) asm volatile("movaps (%2), %%xmm1\n\t" \
		"movups (%1), %%xmm0\n\t" \
		"pcmpgtq %%xmm0, %%xmm1\n\t" \
		"movaps %%xmm1, %%xmm3\n\t" \
		"pshufd $0xee, %%xmm1, %%xmm3\n\t" \
...
protected:
	...
	uint32_t _search_word(T val, T *begin, T *end, search_type type) {
		...
		switch(type) {
			case search_equal:
				for(uint8_t *i = (uint8_t*) begin; i < (uint8_t*) end-16; i += 16) {
					/* check against every possible alignment */
					for(uint8_t j = 0; j < sizeof(T); ++j) {
