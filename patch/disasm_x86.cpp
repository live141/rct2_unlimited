		...
		if(_mod == MOD_REG_DIRECT) {
			...
			if(_rm == 0x04) {
				...
				if(_bitmode == mode_64) {
					if(_is_opsize64()) {
						stream << "rip";
						_operand[i]._base = REGISTER_RIP;
					}
					else {
						stream << "eip";
						_operand[i]._base = REGISTER_EIP;
					}
				}
	...
	for(int i = 0; i < 4; ++i) {
		...
		switch(_code->type_op[i]) {
			...
			case OPERAND_TYPE_AL:
				stream << ", al";
				_operand[i].set_size(1);
				_operand[i]._register = REGISTER_AL;
				break;
			case OPERAND_TYPE_AX:
				stream << ", ax";
				_operand[i].set_size(2);
				_operand[i]._register = REGISTER_AX;
				break;
			case OPERAND_TYPE_EAX:
				if(!_op_size_prefix) {
					stream << ", eax";
					_operand[i].set_size(4);
					_operand[i]._register = REGISTER_EAX;
				}
				else {
					stream << ", ax";
					_operand[i].set_size(2);
			...
			case OPERAND_TYPE_RAX:
				if(_is_opsize64()) {
					stream << ", rax";
					_operand[i].set_size(8);
					_operand[i]._register = REGISTER_RAX;
				}
			...
			case OPERAND_TYPE_DX:
				stream << ", dx";
				_operand[i].set_size(2);
				_operand[i]._register = REGISTER_DX;
				break;
			case OPERAND_TYPE_EDX:
				if(!_op_size_prefix) {
					stream << ", edx";
					_operand[i].set_size(4);
					_operand[i]._register = REGISTER_EDX;
				}
				else {
					stream << ", dx";
					_operand[i].set_size(2);
			...
			case OPERAND_TYPE_RDX:
				if(_is_opsize64()) {
					stream << ", rdx";
					_operand[i].set_size(8);
					_operand[i]._register = REGISTER_RDX;
				}
				else if(_op_size_prefix) {
					stream << ", dx";
					_operand[i].set_size(2);
